...
def render(ctx: dict) -> str:
    ...
    p: list[str] = [
        ...
        f'<p class="meta">生成时间/Generated {esc(ctx["now"])}</p>',
    ]
    ...
    ] + [[f'工具/Tool - {n}', v] for n, v in ctx['tools']]
      + [['宿主机编译副本/Host build copy - ' + s['file'],
          '; '.join(f"`{' '.join(r['old'].split())}` → `{' '.join(r['new'].split())}`：{r['why']}"
                    for r in s.get('replacements', []))]
         for s in spec.get('host_shims', [])], cls='kv'))
    if spec.get('host_shims'):
        p.append('<p class="note">宿主机编译副本由 gen_host_shim.py 按上表逐字替换生成，'
                 '仅用于宿主机构建；覆盖率与诊断通过 #line 归于原文件。'
                 '/Host build copies are generated by gen_host_shim.py with exactly the '
                 'replacements listed; coverage is attributed to the original file via #line.</p>')
...
def main() -> int:
    ...
    out = args.out or (args.root / spec.get('output_dir', 'unit_test')
                       / 'report' / 'unit_verification_report.html')
    out.parent.mkdir(parents=True, exist_ok=True)

    title = f"{spec.get('component', '')} 软件单元验证报告 / Unit Verification Report"
