...
5. **SDK 与原有模块/接口只增不删（基于项目 SDK 生成代码时务必满足）**——基于项目已有 SDK 生成代码时，
   **绝不删除原有的模块代码与接口代码**，也**绝不对 SDK 本身做任何删减**（不删文件、不删函数、不删接口、不改其公开签名/语义）。
   允许**按选定架构来源做重构**：在分层目录里重新组织、为现有接口补适配/门面（HAL `If/Impl`、封装层）、新增模块或接口、
   把 SDK 接口包到抽象层之后以便注入打桩；但重构只能"包裹/新增/搬位置"，不能"删减/改写" SDK 与原有接口。
   原 SDK 目录内若存在**明显放错层**的模块（例如业务逻辑落在 driver/MCAL 目录、板级 HAL 实现混在 App、OSIF 混在 MCAL），
   可以按选定架构搬到正确层；这属于架构纠错，不属于删除。搬层时必须保留文件内容与公开接口兼容，更新 `path_map`、
   include 路径和构建脚本，记录“原路径 → 新路径、原层 → 新层、判定理由”，并通过 `check_layering.py`。
   若架构确实要求改变某个原有接口的用法，保留原接口、另加新接口或适配层，并在 codegen plan/架构决策记录里写明理由。
...
- **读 SDK 代码根**：盘点已有的 BSP/MCAL/OS/RTOS API 与已存在的 HAL/驱动接口——生成代码要**对接这些已有接口**，
  ...
  **只增不删（硬性要求 5）**：盘点是为对接，不是为替换——**绝不删除原有模块/接口代码，绝不删减 SDK**
  （不删文件/函数/接口，不改 SDK 公开签名与语义）。要适配架构就**新增适配层/门面或包裹现有接口**，
  把"重构"限制在重组目录、加抽象层、加新接口上；保留所有既有接口可用。
  若发现原 SDK 中模块物理目录与职责/依赖明显错层，允许搬到正确层，但要先在 codegen plan 里列出迁移清单：
  `module`、`old_path`、`old_layer`、`new_path`、`new_layer`、`reason`、`compatibility_strategy`。
  搬层后同步更新 `layers.json.path_map`、include 路径、构建文件与文档，必要时保留旧路径转发头/兼容 wrapper，保证既有调用不突然失效。
