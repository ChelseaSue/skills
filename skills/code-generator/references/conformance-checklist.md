...
## A2. SDK 与原有模块/接口保全（只增不删，硬性要求 5）
- [M] 基于项目 SDK 生成时，**原有模块代码与接口代码均未被删除**；既有公开接口仍存在且可用。
- [M] **SDK 本身未被删减**：未删 SDK 文件/函数/接口，未改 SDK 公开签名与语义（对接而非替换）。
- [M] 架构所需的改动以**重组目录 / 新增模块或接口 / 适配层 / 门面 / 包裹现有接口**实现，而非删改 SDK 或原有接口。
- [M] 对原 SDK 中明显错层模块的搬迁已有迁移清单：`原路径→新路径`、`原层→新层`、判定理由、兼容策略；
  搬迁后 `path_map`、include 路径、构建脚本和文档均已同步。
- [A] 搬层后 `check_layering.py` 为 0 违规，且无因旧路径残留导致的重复归层/未归层文件。
- [M] 若架构要求改变某原有接口用法，已保留原接口并另加新接口/适配层，且在 codegen plan/架构决策记录里写明理由。
